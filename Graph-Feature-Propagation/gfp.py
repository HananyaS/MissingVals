...
class GFP:
    ...
    def __init__(self, index=False, iters=50, eps=1e-3, delta_func=rmse, early_stop=True):
        """
        ...
        self.index = torchify(index, self.device).bool() if type(index) != BOOL else index
    ...
    def define_index(self, X):
        ...
        if type(self.index) != BOOL:
            return None
        self.index = (X == 0).bool() if type(X) == TNSR else torch.from_numpy(X == 0).bool()
        return None
    ...
    def prop(self, X, edges):
        ...
        for i in range(self.iters):
            ...
            self.delta.append(self.f(AX[self.index == False], old_mat[self.index == False]))
